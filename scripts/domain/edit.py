...
if __name__ == '__main__':
    ...
    print(f'{Color.fg.green}[+] {len(out[1:])} domains found:{Color.reset}\n')
    lines = []
    for i in out[1:]:
        item = []
    ...
    id = input(f'\n{Color.fg.cyan}[>] Enter the id of the domain that you want to edit: {Color.reset}')
    domain = input(f'{Color.fg.cyan}[>] Enter the domain that you want to set: {Color.reset}')

    command_lines = [
        'mysql -u root',
        'USE mailserverdb;',
        f'UPDATE virtual_domains SET name = \'{domain}\' WHERE id = {id};',
        'quit'
    ]
    _ = run_sql_query(command_lines, '[-] Error while updating domain:')
    print(f'{Color.fg.green}[+] Domain updated successfully.{Color.reset}')
