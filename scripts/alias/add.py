...
if __name__ == '__main__':
    ...
    domain_id = input(f'{Color.fg.cyan}[>] Enter the domain id for which you want to add alias: {Color.reset}')
    ...
    command_lines = [
        'mysql -u root',
        'USE mailserverdb;',
        f'INSERT INTO virtual_aliases VALUES({int(out[-1][0]) + 1}, {domain_id}, \'{source}\', \'{destination}\');',
        'quit'
    ]
    _ = run_sql_query(command_lines, f'[-] Error while adding alias to `virtual_aliases`:')
    print(f'{Color.fg.green}[+] Alias added to `virtual_aliases`.{Color.reset}')
